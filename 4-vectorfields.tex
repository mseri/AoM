...
\begin{proposition}\label{prop:vfderivationiso}
...
\begin{proof}
	...
	\end{equation}
	We can then think of $X$ as a map $W\to TW$ via $p\mapsto X_p$.
...
\begin{equation}\label{def:commutator}
	[X,Y] := X Y - Y X.
\end{equation}
Indeed, $[X,Y]$ is a derivation, and so by Proposition~\ref{prop:vfderivationiso} this also defines a vector field.

\marginnote{Do not confound Lie with lie. Here Lie is the surname of \href{https://www.wikipedia.org/wiki/Sophus_Lie}{Sophus Lie}, an important Norwegian mathematician that lived in the second half of the 19th century.}
