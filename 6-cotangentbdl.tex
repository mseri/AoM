...
\begin{definition}\idxdef{Dual space}
...
\begin{definition}\idxdef{Cotangent space}
...
\begin{remark}[The double dual]\label{rmk:double_dual}\idxdef{Double dual}
...
\begin{definition}\label{def:pullback:oneform}\idxdef{Pullback of covectors}
...
\begin{definition}\idxdef{Cotangent bundle}
...
\begin{definition}\label{def:covfield}\idxdef{Covector field}\idxdef{Differential one-form}
...
\begin{exercise}
	A section $\omega$ of $T^*M$ is smooth, and thus a differential form, if and only if, for every smooth vector field $X\in\fX(M)$, the function $(\omega \mid X)\in C^\infty(M)$.
	\textit{\small Hint: write it down in local coordinates.}
\end{exercise}

\begin{definition}\label{def:pullback1f}\idxdef{Pullback of one-forms}
...
\begin{exercise}
	...
	For $p\in M$, denote $(V, (y^i))$ a chart on $N$ around $F(p)$ and let $U=F^{-1}(V)$.
...
\begin{example}[The pullback bundle]\idxdef{Pullback bundle}
...
\begin{definition}\idxdef{Line integral of one-forms along curves}
