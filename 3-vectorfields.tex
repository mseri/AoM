...
\begin{proof}
  ...
  \begin{align}
    X(f\circ F)(p) = X_p(f\circ F) = dF_p(X_p)f = (F_* X)_{F(p)}f = (F_*X f)(F(p)) = (F_*X f)\circ F(p).
  \end{align}
...
\begin{notation}
  ...
  One particularly interesting consequence of this notation is the following:
  \begin{equation}
    \left(e_*^{tX} Y\right)_q = e_*^{tX} \left( Y\big|_{e^{-tX}(q)} \right) =
    \frac{d}{ds}\Big|_{s=0} e^{tX}\circ e^{sX}\circ e^{-t X}(q),
  \end{equation}
  which turns out to be a rather useful formula.
\end{notation}
...
\begin{proposition}
  ...
  \marginnote{Sometimes to understand what one is doing, it may be convenient to rewrite things in different forms, for example I find the following quite clarificatory \begin{equation}\Phi_t^{F_* X} = F \circ \Phi_t^X \circ F^{-1},\end{equation}}
