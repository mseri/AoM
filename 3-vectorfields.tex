...
\begin{definition}
  ...
  We denote\footnote{Since vector fields on a manifolds are also tensor fields of type $(1,0)$, a common equivalent notation would be $\cT_0^1(M)$. This note will become clear later on, after we introduce tensors.} the set of smooth vector fields by $\fX(M)$.
\end{definition}
...
The most natural choice is to just apply the derivatives one after the other:
\begin{equation}
  X Y: C^\infty(W) \to C^\infty(W), \quad
  (X Y)(f) := X(Y(f)).
\end{equation}
