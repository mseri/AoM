...
\begin{example}\label{ex:rotation}
  ...
  If $\gamma:\R^2\to\R$ is a smooth curve, written in standard coordinates as $\gamma(t) = (x(t), y(t))$, then the condition~\eqref{eq:integralCurve} for $Z$ and $\gamma$ translates into
  \marginnote{Make sure that you have understood why this is true.}
  \begin{equation}
...
\begin{theorem}[Existence, uniqueness and differentiability of local solutions] %
  \label{thm:exuniqloc}
  Let $M$ be a smooth manifold and $X\in\fX(M)$. For every $x\in \mathring M := M\setminus \partial M$, there exists $\epsilon > 0$, an open neighbourhood $U\subset\mathring M$ of $x$ and a unique map $\varphi : (-\epsilon, \epsilon) \times U \to M$, $(t, p) \mapsto \varphi(t,p)$, such that
  \begin{enumerate}[(i)]
...
\begin{theorem}[Existence and uniqueness of maximal solutions] %
  \label{thm:exuniqmax}
  Let $M$ be a smooth manifold and $X\in\fX(M)$.
  There exists a unique open set $\cD \subset \R\times\circ M$ and a unique smooth map $\varphi:\cD\to M$ such that
  \begin{enumerate}[(i)]
...
%   Both $\cD$ and $\varphi$ are uniquely identified respectively by (\ref{thm:exuniqmax:i}) and (\ref{thm:exuniqmax:ii}), so we only need to show that $\cD$ is open and that $\varphi$ is smooth.
%   To this end we are going to prove that for all $p\in M$ and $t\in I_p$, $\cD$ contains a neighbourhood of $(t,p)$ on which $\varphi$ is smooth.

%   Fix $p\in M$ and denote $J\subset I_p$ the set of all $t\in I_p$ for which there is some neighbourhood of $(t,p)$ contained in $\cD$ on which $\varphi$ is smooth. Since this is an open condition, the set $J$ is open and by Theorem~\ref{thm:exuniqloc}, $0\in J$, so $J$ is not empty.
...
%   Applying Theorem~\ref{thm:exuniqloc} at $p_0$ we obtain a local flow $\varphi:(-\epsilon,\epsilon)\times U_0 \to M$ about $p_0$. 
...
%   By the continuity of $\varphi$ at $(t_1, p)$, there exists a neighbourhood $V$ of $p$ such that $\varphi(\{t_1\}\times V)\subset U_0$.

%   Let $t\in I_0$ and $q\in V$, then $t-t_1\in(-\epsilon, \epsilon)$ and thus $\varphi(t_1,q)\in U_0$ (it coincides with $\varphi$ there).
%   But this implies that $\varphi(t-t_1, \varphi(t_1, q))$ is defined and smooth.
%   Since the curve $s\mapsto \varphi(s-t_1, \varphi(t_1, q))$ is an integral curve of $X$ passing through $\varphi(t_1, q)$ at $t_1$, by uniqueness, the curve must be $\varphi(t,q)$ itself.
%   That is, $\varphi(t,q) = \varphi(t-t_1, \varphi(t_1, q))$ is defined and smooth at $(t,q)$.
%   This shows that $\varphi$ is defined and smooth on all $J_0\times V$ and, in particular, also that $t_0\in J$.
\end{proof}
...
%\section{Infinitesimal generators} % TODO: for next year add this section title
...
\begin{definition}
  ...
  If $\{\varphi_t\}$ is a one-parameter group of diffeomorphisms, then we define its \emph{infinitesimal generator} as the (complete) vector field\footnote{In a more compact form, we have just written $X_p = \frac{d}{dt}|_{t=0}\varphi_t(p)$.}
...
\begin{proposition}\label{prop:conjpfX}
  ...
  \end{equation}
  \marginnote[-6em]{Sometimes to understand what one is doing, it may be convenient to rewrite things in different forms, for example I find the following quite clarificatory \begin{equation}\label{eq:conjpfX}\varphi_t^{F_* X} = F \circ \varphi_t^X \circ F^{-1},\end{equation}}
\end{proposition}
\begin{proof}
...
\newthought{We can finally justify} our comment in Figure~\ref{fig:3_1-vfield-infgen}: the following theorem shows that every flow $\varphi^X$ admits local coordinates on its support which map it into a linear flow.
