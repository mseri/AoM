...
\begin{theorem}[Existence, uniqueness and differentiability of local solutions] %
  \label{thm:exuniqloc}
  Let $M$ be a smooth manifold and $X\in\fX(M)$. For every $x\in \mathring M := M\setminus \partial M$, there exists $\epsilon > 0$, an open neighbourhood $U\subset\mathring M$ of $x$ and a unique map $\Phi : (-\epsilon, \epsilon) \times U \to M$, $(t, p) \mapsto \Phi(t,p)$, such that
  \begin{enumerate}[(i)]
...
Note that $I_p$ is typically larger than the domain of definition $(-\epsilon, \epsilon)$ of $\Phi_p$. By construction $\Phi_p$ never leaves the coordinate set $U$ over which it was defined, on the other hand $\gamma_p$ can in general wander all over the manifold\todo{Add picture!} -- even though $\gamma_p = \Phi_p$ for values of $t$ small enough!
...
\begin{exercise}
  ...
  Moreover, if $X(x) = Ax$ is a linear vector field on $\R^n$, i.e., $A$ is a $n\times n$-matrix, then the corresponding flow $\Phi_t$ is the matrix exponential $\Phi_t(x) = e^{Ax}$.
\end{exercise}
...
\end{theorem}
\begin{proof}
  Choose\todo{Add clarification note on how to construct it and example.} a coordinate patch $(U_1, \psi)$ centered at $p$ (so $\psi(p) = 0$) such that $\psi_* X(0) = (1,0,0,\ldots, 0)$.
