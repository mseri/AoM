...
\begin{exercise}
  Show that the Lie brackets satisfy the following properties. Let $X,Y,Z\in\fX(M)$:
  \begin{enumerate}[(i)]
    \item antisymmetry $[X, Y] = - [Y, X]$;
    \item bilinearity $[X + \alpha Y, Z] = [X, Z] + \alpha [Y, Z]$ and $[X, Y + \alpha Z] = [X, Y] + \alpha [X, Z]$, for all $\alpha\in\R$;
...
\begin{definition}
  ...
  \begin{marginfigure}
    ...
    \caption{The name ``infinitesimal generator'' comes from the intuition that one can think of a flow as a sequence of infinitely many infinitesimally small linear steps. That is, an integral curve is well approximated on local charts by composing many small rectilinear motions, where the direction and length of each motion is determined by the value of the vector field at the point arrived at in the previous step. We will soon make this step rigorous.}
  \end{marginfigure}
  If $\{\Phi_t\}$ is a one-parameter group of diffeomorphisms, then we define its \emph{infinitesimal generator} as the (complete) vector field\footnote{Remember our previous remarks on the notation: this is just a scary way to write $X_p := \Phi_p'(0)$ that explicits the role of the differential and the nature of the vector field!}
  \begin{equation}
...
\begin{notation}
  ...
  \begin{equation}
    e^{t X} \cdot := \Phi^X_t(\cdot), \quad t\in\R,
  \end{equation}
...
\begin{exercise}
  ...
  \begin{align}
    e^{0X} = \id_M, \quad (e^{tX})^{-1} = e^{-tX}, \quad e^{t X}\circ e^{s X}= e^{(t+s)X},\\
    \frac{d}{d t} e^{tX} (p) = X(e^{tX}(p)), \quad \forall p\in M.
  \end{align}

  Moreover, if $X(x) = Ax$ isd a linear vector field on $\R^n$, that is $A$ is a $n\times n$-matrix, then the corresponding flow $\Phi_t$ is the matrix exponential $\Phi_t(x) = e^{Ax}$.
\end{exercise}
...
We can finally justify our comment in Figure~\ref{fig:3_1-vfield-infgen}.
...
\begin{theorem}[Normal form of a flow away from the fixed points]
  ...
  \begin{equation}
    \phi_* X = L,
  \end{equation}
  where $L$ is the linear drift defined in Example~\ref{eq:lineardrift}.
  Thus, locally,
  \begin{equation}
...
\begin{proof}
  ...
  Our objective is to interpolate $\psi_* X$ on $V\subset V_2$ and $L$ on $V_2^c$ to obtain a new vector field $\tilde L$ on all of $\R^n$ which is diffeomorphic to $\tilde L$.
  If $\Omega:\R^n\to\R^n$ is such diffeomorphism, i.e. $\Omega_* \tilde  L = L$, then $\phi = \Omega\circ\psi$ is the chart we are looking for: indeed, on $U=\psi^{-1}(V)$ we have $\phi_* X = \Omega_*\psi_*X = \Omega_* \tilde L = L$.

  \newthought{Interpolated vector field $\tilde{L}$}. Let $V\subset V_2$ be an open set around $0$. Pick a cutoff function $\chi\in C^\infty(\R^n)$ such that $\chi(x) = 1$ for $x\in U$ and $\chi(x) =0$ for $x\in U_2^c$.
  Then the interpolating vector field is immediately obtained as\todo{add picture}
  \begin{equation}
    \tilde L = \chi \psi_*X + (1-\chi)L \fX(\R^n).
  \end{equation}
  Clearly its flow $\Phi_t^{\tilde L}$ is global as $\R^n$ has no boundary and the regularity of the functions involved implies that no integral curve can escape to infinity in finite time.
  ...
  \begin{equation}
    \Omega = \lim_{t\to\infty} \Phi_{-t}^L \circ \Phi_t^{\tilde L}
  \end{equation}
  exists and defines a diffeomorphism such that $\Omega_* \tilde L = L$.
  Since by construction $(\Phi_t^{\tilde L}(q))^1 \geq q^1 + \frac12$, every integral curve leaves $V_2$ after a finite time.
  So, on compact sets $K\subset\R^n$, there is a finite time $t_0(K)$ after which the limit is attained (can you explain why?):
  \begin{equation}
    \lim_{t\to\infty} \Phi_{-t}^L \circ \Phi_t^{\tilde L}\Big|_K = \Phi_{-t_0}^L \circ \Phi_{t_0}^{\tilde L}.
  \end{equation}
  Thus, $\Omega$ is a well defined diffeomorphism.
  Moreover,
  \begin{align}
    \Omega \circ \Phi^{\tilde L}_t &= \lim_{s\to\infty} \Phi_{-s}^L \circ \Phi^{\tilde L}_s \circ \Phi^{\tilde L}_t \\
    &= \lim_{s\to\infty} \Phi_{-s}^L \circ \Phi^{\tilde L}_{s+t} \\
    &= \lim_{\tau\to\infty} \Phi_{t-\tau}^L \circ \Phi^{\tilde L}_s \circ \Phi^{\tilde L}_\tau \\
    &= \lim_{\tau\to\infty} \Phi_{t}^L \circ \Phi_{-\tau}^L \circ \Phi^{\tilde L}_s \circ \Phi^{\tilde L}_\tau = \Phi_t^L \circ \Omega,
  \end{align}
  and therefore the flows are diffeomorphic.
  In particular, we also have $\Phi_p^{\tilde L} = \Omega^{-1} \circ \Phi^L_{\Omega(x)}$.
  Differentiating this last equation we get
  \begin{align}
    (\tilde L \circ \Phi^{\tilde L})_p &= d\Phi^{\tilde L}_p \\
    &= d\Omega^{-1}_(\Phi^L_{\Omega(x)}) \circ d\Phi^L_{\Omega(x)} \\
    &= d\Omega^{-1}_(\Phi^L_{\Omega(x)}) \circ (L \circ \Phi^L)_{\Omega(x)},
  \end{align}
    which, at $t=0$, gives $\tilde L = d\Omega^{-1} \circ L \circ \Omega = \Omega^{-1}_* L$.
\end{proof}
...
\begin{remark}[Linearisation of a vector field at a fixed point]
  ...
  Qualitatively, the behaviour close to the fixed point is determined by the eigenvalues of $d\tilde X_0 x$ and their geometric multiplicities (cf. \cite[Figure 9.8]{book:lee}). This is the same as you have seen in the euclidean case \cite[Chapter 5.3]{book:knauf}.

  Note that thanks to the compatibility conditions, the linearisations of $X$ at different charts are similar matrices, and thus the spectrum of $d\tilde X_0 x$ is independent of the choice of local coordinates.
\end{remark}

For simplicity we discuss only global flows in the following.
