...
\begin{proof}
  ...
  If $\Omega:\R^n\to\R^n$ is such diffeomorphism, i.e. $\Omega_* \widetilde  L = L$, then $\phi = \Omega\circ\psi$ is the chart we are looking for: indeed, on $U=\psi^{-1}(V)$ we have $\phi_* X = \Omega_*\psi_*X = \Omega_* \widetilde{L} = L$.
...
\begin{remark}[Linearisation of a vector field at a fixed point]
  ...
  If $X\in\fX(M)$ and $p_0\in M$ with $X_{p_0} = 0$. On a chart $\phi$ centred at $p_0$ with local coordinates $(x^i)$, let $\widetilde X = \phi_* X$.
  Then we have
  \begin{equation}
