...
\begin{notation}
  ...
  \begin{equation}
    \frac{\partial}{\partial t}\Phi_t(p) = X(\Phi_t(p)), \quad
...
\begin{exercise}
  ...
  \begin{align}
    ...
    \frac{\partial}{\partial t} e^{tX} (p) = X(e^{tX}(p)), \quad \forall p\in M.
  \end{align}
...
\begin{corollary}
  Let $X\in\fX(M)$ be a vector field with compact support, that is, such that
  \begin{equation}
    \supp(X):=\overline{\left\lbrace p\in M \;\mid\; X_p\neq 0 \in T_p M\right\rbrace}\subset M
  \end{equation}
  is a compact set. The $X$ is complete.
\end{corollary}
...
\begin{corollary}
  If $M$ is compact, then every vector field has compact support.
\end{corollary}

In the same fashion as Lemma~\ref{lemma:charactCompleteVF}, one can characterize non-complete vector fields.

\begin{proposition}
  Let $K\subset \circ M$ compact and $p\in K$. If $t^+(p) < \infty$, then there exists $0<\tau<t^+(p)$ such that $\gamma_p(t)\not\in K$ for all $t\in (\tau, t^+(p))$. An analogous statement holds if $t^-(p) > -\infty$.
\end{proposition}

In other terms, a maximal integral curve can not ``end'' inside a compact set that doesn't contain boundary points.
