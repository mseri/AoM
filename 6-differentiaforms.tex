...
\begin{definition}
  ...
  \end{equation}
  The subspace of alternating tensors in $T_k^0(V)$ is denoted by $\Lambda^k \equiv \Lambda^k(V) \subset T_k^0(V)$ and its elements are called \emph{exterior forms}, \emph{alternating $k$-forms} or just  \emph{$k$-forms}.
...
\begin{definition}
  ...
  Let $(U,\phi)$ denote a chart on $U\subset M$ with local coordinates $(x^i)$.
  Then, the \emph{exterior derivative} is defined locally as $d\omega|_U := \phi^*d(\phi_* \omega)$, that is, for
  \begin{equation}
...
\begin{theorem}\label{thm:differentialpushforward}
  Let $M$ be a smooth $n$-manifold, $(U,\phi)$ a chart on $U\subset M$ and $F:M\to N$ a diffeomorphism between smooth manifolds.
  Then, for $\omega\in\Omega^k(N)$, we have $F^*(d\omega|_{F(U)}) = dF^*\omega|_U$.
\end{theorem}
\begin{proof}
  Let $(x^i)$ denote the local coordinates of $\phi$ and let $(\widetilde U, \widetilde\phi) := (F(U), \phi\circ F^{-1})$ be the pushforward of the chart to $F(U)\subset N$ with its local coordinates $(y^i)$ on $N$.
...
\begin{corollary}
  Let $M$ be a smooth $n$-manifold and $(U_i, \phi_i)$, $i=1,2$, two charts on $M$.
  Then, for $\omega\in\Omega^k(M)$, the following holds
  \begin{equation}
    \phi_1^*\left(d(\phi_{1*}\omega)_{\phi_1(U_1\cap U_2)}\right) =
    \phi_2^*\left(d(\phi_{2*}\omega)_{\phi_2(U_1\cap U_2)}\right).
  \end{equation}
  Therefore, the exterior derivative $d\omega\in\Omega^k(M)$ is uniquely defined by the local definition~\eqref{eq:localdw}.
\end{corollary}
\begin{proof}
  Follows from Theorem~\ref{thm:differentialpushforward} applied with $F = \phi_1\circ\phi_2^{-1} : \phi_2(U) \to \phi_1(U)$, where $U=U_1\cap U_2$.
  Indeed, since by the chain rule $F^* = (\phi_2^{-1})^*\phi_1^* = \phi_{2*}(\phi_1^{-1})_*$, we have
  \begin{align}
    \phi_1^*\left(d(\phi_{1*}\omega)_{\phi_1(U)}\right)
    &= \phi_2^* (\phi_2^{-1})^* \phi_1^*\left(d(\phi_{1*}\omega)_{\phi_1(U)}\right) \\
    &= \phi_2^* F^*\left(d(\phi_{1*}\omega)_{\phi_1(U)}\right) \\
    &= \phi_2^* \left(d(F^*\phi_{1*}\omega)_{\phi_2(U)}\right) \\
    &= \phi_2^* \left(d(\phi_{2*}\omega)_{\phi_2(U)}\right).
  \end{align}
\end{proof}
...
\begin{theorem}\label{thm:LieDerivativeFlow}
  Let $M$ be a smooth complete $n$-manifold, $X\in\fX(M)$ and $\Phi_t$ its flow.
  Then, for all $\omega\in \Omega^*(M)$, one has
  \begin{equation}
    \frac{d}{dt}(\Phi_t^* \omega) = \Phi_t^*\cL_X\omega.
  \end{equation}
\end{theorem}
\begin{proof}
  \newthought{Step I}.
  ...
  \begin{align}
    ...
    &= \frac{d}{ds}(\Phi_{t+s}^* \omega)\Big|_{s=0} \\
    &= \frac{d}{ds}(\Phi_t^*\Phi_s^*\omega)\Big|_{s=0} \\
    &= \Phi^*_t \frac{d}{ds}(\Phi_s^*\omega)\Big|_{s=0}.
  \end{align}
  ...
  \begin{align}
    \frac{d}{dt}\Big|_{t=0}\Phi_t^* f(x) 
    &= \lim_{t\to0} \frac{f(\Phi_t(x)) - f(t)}{t} \\
  ...
  \begin{align}
    \frac{d}{dt}(\Phi_t^* dx^i)\Big|_{t=0}
    &= \frac{d}{dt}(d\Phi_t^* x^i)\Big|_{t=0} \\
    &= d\, \frac{d}{dt}(\Phi_t^* x^i)\Big|_{t=0} \\
    &= dX^i.
  \end{align}
  On the other hand,
  \begin{align}
...
\begin{remark}
  ...
  \begin{equation}
    (\cL_X T)_p := \frac{d}{dt}\Big|_{t=0}\left((\Phi_t^X)^* T\right)_p,
  \end{equation}
  where as usual $\Phi_t^X$ denotes the maximal integral curve\footnote{Remember, this is a diffeomorphims from a neighbourhood of $p$ onto a neighbourhood of $\Phi_t^X(p)$.} for $X$ with initial point $p$.
...
\end{theorem}
\begin{proof}
Let $\phi: U \to V\subset\R^n$ be a diffeomorphism between $U$ and the star-shaped domain $V\subset\R^n$.
Then $\widetilde\omega := \phi_*\omega$ is a closed $k$-form on $V$ and, according to the Poincar\'e lemma on $\R^n$, there exists $\widetilde \nu \in \Omega^{k-1}(V)$ such that $\widetilde\omega = d\widetilde\nu$.
\end{proof}
...
\begin{proof}
  ...
  Let $\Phi_t$ now denote the flow of $T$, then $\Phi_t(s, p) = (t+s, p)$ and thus $j_t = \Phi_t \circ j_0$.
  By Theorem~\ref{thm:LieDerivativeFlow} we can compute the integrand as
  \begin{align}
    j_t^*(\cL_T(\omega)) &= j^*_0(\Phi_t^*(\cL_T(\omega))) \\
    &= j_0^*\left(\frac{d}{dt} \Phi_t^*(\omega)\right) \\
    &= \frac{d}{dt} j_0^*(\Phi_t^*(\omega)) \\
    &= \frac{d}{dt} j_t^*(\omega).
  \end{align}
