...
\begin{example}[Exterior derivatives and vector calculus in $\R^3$]
  ...
  \begin{equation}
    % https://q.uiver.app/?q=
    \begin{tikzcd}
      {\fX(M)} \\
  & {\Omega^1(M)} &&&& {\Omega^2(M)} \\
  \\
  ...
  \arrow["d", shift right=1, from=4-3, to=2-2]
  \arrow["d", shift left=1, from=2-2, to=2-6]
  \arrow["\star", tail reversed, from=4-5, to=4-3]
  \arrow["\star"', shift right=1, tail reversed, from=2-2, to=2-6]
  \arrow["d", from=2-6, to=4-5]
    \end{tikzcd}
  \end{equation}
  Walking the diagram, one can directly generalize $\nabla$, $\nabla \cdot$ and $\nabla\times$ to $3$-manifolds:
  \begin{itemize}
    \item $\mathrm{grad} := \sharp \circ d : \Omega^0(M) \to \fX(M)$;
    \item $\mathrm{div} := \star d \star \flat : \fX(M) \to \Omega^0(M)$;
    \item $\Delta := \mathrm{div} \circ \mathrm{grad} = \star d \star d : \Omega^0(M) \to \Omega^0(M)$;
    \item $\mathrm{curl} := \sharp \circ \star \circ d \circ \flat : \fX(M) \to \fX(M)$.
  \end{itemize}
  For a more pictorial presentation of this material, refer also to the youtube video \href{https://www.youtube.com/watch?v=ZpUvFn8Ni2I}{G. Bracchi - There is only one derivative: the exterior derivative}.
