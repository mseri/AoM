...
As we were saying, now we can take the tensor product of two forms $\omega\otimes\nu$ and use the antisymmetrisation $\Alt_{k+h}$ to to project it onto the antisymmetric subspace $\Lambda_{k+h}$ of $T_{k+h}^0(V)$.
...
By convention $i_v\omega = 0$ if $\omega\in\Lambda_0$.
...
The subset of $\cT_k^0(M)$ consisting of alternating $k$-tensors is denoted by $\Lambda_k(M):= \bigsqcup_{p\in M} \Lambda_k(T_p M)$.
\begin{definition}
