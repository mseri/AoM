...
\begin{ex}
  ...
  \begin{prop}
    Let $M, N, P$ be three $C^r$-manifolds, and suppose that $\phi:M\to N$ and $\psi:N\to P$ are of class $C^r$.
    Then $\psi\circ\phi\in C^r(M, P)$.
  \end{prop}  
\end{ex}
...
Of course this is a rhetorical question. We can generalize our definition by considering the \emph{closed upper half-spaces}
\begin{equation}
...
\begin{defn}
  ...
  \begin{enumerate}[(i)]
    \item $M$ is a Hausdorff space;
    \item $M$ is second countable;
    \item $M$ is \emph{locally} homeomorphic to $\cH^n$, any point $x\in M$ has a neighborhood that is homeomorphic to a relatively open\footnote{Recall that $U\subset\R^n_+$ is relatively open if there exist an open set $\tilde U\subset\R^n$ such that $U = \tilde U \cap \R^n_+$.} subset of $\R^n_+$.
  \end{enumerate}
\end{defn}

\subsection{Infinite dimensional manifolds $\star$}
\todo{See Merry, Remark 1.31.}
