...
\paragraph{High school.} A function $f:\R\to\R$ is \emph{continuous} if you can draw it without lifting your pen from the page.

\paragraph{Analysis.} A function is continuous if its left and right limits at each point exist and have the same value.

\paragraph{Metric and topological spaces.} A map $f:X\to Y$ between \emph{topological spaces} is continuous if preimages of open sets under $f$ are open. More explicitly, $f$ is continuous if for every open set $O\subset Y$, $f^{-1}(O)\subset X$ is an open set.
\medskip

The latter immediately generalizes also to higher dimensional spaces, and can be directly taken as definition for \emph{Multivariable Analysis}. This already hints at the need to maintain topological spaces as a central underlying structure for our further discussions.
\medskip

\newthought{Central to analysis is also the notion of derivation}, and then things get more interesting and challenging. Again, let's step trough the various stages of abstraction you likely saw until now.
\medskip

\paragraph{High school.} The derivative $f'(x)$ of $f$ at a point $x$ is just the slope of the function $f$ at the point $x$.

\paragraph{Analysis.} We make it more precise by saying that $f:\R\to\R$ is \emph{differentiable} at a point $x$ if the limit
\begin{equation}
	f'(x) := \lim_{h\to0} \frac{f(x+h) - f(x)}{h}
\end{equation}
exists, defining in this way its derivative $f'$.

This new function $f'$ does not need to be continuous, so we say that $f$ \emph{continuously differentiable} if $x\mapsto f'(x)$ is itself a continuous function.

\paragraph{Multivariable analysis.} Now things start getting interesting. To generalize derivatives to functions with more than one variable we suddenly need more structure: a continuous function $f=(f^1, \ldots, f^m):\R^n\to\R^m$ is \emph{differentiable} at $x=(x^1,\ldots,x^n)\in\R^n$ if there is a \emph{linear map}\footnote{That is, $T$ is a $m\times n$ matrix with respect to some chosen basis.} $T: \R^n\to\R^m$ such that
...
As we discussed above, the definition of continuity is extremely general. And defining limits in general is not a problem\footnote{They can be defined \href{https://en.wikipedia.org/wiki/Filter_(mathematics)\#Examples}{very abstractly in terms of filters}.}. Moreover, if $X$ and $Y$ are \emph{metric spaces}, the definition of continuity reduces to the definition given above for instance.
But how can we make sense of differentiability in the general case?

If you have taken a course on \emph{Calculus of Variations}, you probably know that you can make sense of~\eqref{eq:diff} and give a notion of differentiability in the case $X$ and $Y$ are Banach spaces\footnote{Complete normed vector spaces, possibly infinite dimensional.}.
But, in general, a topological space is \emph{not} a vector space: there is no notion of adding points and, least of all, one of linearity.

\newthought{This is where differential geometry comes into play}.
...
\begin{remark}
	The union of all atlases in a differentiable structure is the \emph{unique} \emph{maximal} atlas in the equivalence class.
	There is a one-to-one correspondence between differentiable structures and maximal differentiable atlases \cite[Proposition 1.17]{book:lee}.
\end{remark}

\begin{tcolorbox}
	For convenience and to lighten the notation, from now on, we will always regard a differentiable structure as a differentiable maximal atlas without further comments.
\end{tcolorbox}
...
\begin{example}
	...
	In fact, we can define the differentiable manifold $\bS^1$ by equipping the topological quotient space\sidenote[][-14em]{
...
\section{Quotient manifolds}\label{sec:quotient}
...
\begin{exercise}[Equivalent definitions of smoothness]\label{prop:eq-def-smooth}
	...
	\begin{enumerate}
		\item for every $p\in M_1$, there are smooth charts $(V_1,\phi_1)$, $p\in V_1$, and $(V_2,\phi_2)$, $F(p) \in V_2$, such that $F(V_1) \subseteq V_2$ and $\phi_2 \circ F \circ \phi_1^{-1}$ is smooth from $\phi_1(V_1)$ to $\phi_2(V_2)$;
