...
\begin{remark}\label{remark:smoothstr}
  ...
  The case $n = 4$ is unknown: if you prove that there is only one smooth structure, you will have shown the smooth Poincar\'e conjecture \cite{notes:calegari}.
\end{remark}
...
\begin{proof}
  ...
  In particular, the square matrix $D\tau_{12}|_{\phi_1(p)}$ is inveritible and, as such, defines an isomorphism.
  The inverse function theorem then implies that $\tau_{12}(B)$ (here we consider $\tau_{12}$ as a map from $B$ to $\R^n$) is an open subset in $\R^n$ containing $\phi_2(p)$ and contained in $\phi_2(U_2)$.
