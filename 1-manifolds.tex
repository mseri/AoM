...
\begin{example}
  ...
  \begin{itemize}
    ...
    \end{equation}
    then $x = \phi_2(p) = (\sigma\circ\psi)(p)$, $\phi_2(U_2) = \R\times[0,1/2) \subset\cH^2$.
...
\begin{exercise}
  Explicitly define $\phi_3$ from the previous example, make a plot of all the three charts and show that the boundary is indeed $\partial\cC = \phi_2^{-1}(\R\times\{0\}) \cup \phi_3^{-1}(\R\times\{0\})$ as claimed.
\end{exercise}
