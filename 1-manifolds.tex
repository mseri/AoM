...
EXERCISES\todo{Maybe add section homework sheet}
...
\begin{defn}
  Let $M$ be a smooth manifold. A \emph{partition of unity} is a collection $\{\lambda_\alpha \mid \alpha\in A\}$ of functions $\lambda_\alpha:M\to\R$ such that
  \begin{enumerate}
    \item $0 \leq \'lambda_\alpha \leq 1$ for all $p\in M$ and $\alpha\in A$;
    \item the collection $\{\lambda_\alpha \mid \alpha\in A\}$ is \emph{locally finite}, that is, for any $p\in M$ there are at most finitely many $\alpha\in A$ such that $p\in\supp(\lambda_\alpha)$;
    \item for all $p\in M$ one has $\sum_{\alpha\in A} \lambda_\alpha(p) = 1$.
  \end{enumerate}
\end{defn}