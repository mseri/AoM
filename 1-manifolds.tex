...
\begin{quote}
  ...
  Let $x^i = r^i \circ\phi$ denote the components of $\phi$ with respect to the standard euclidean coordinates\ldots
\end{quote}
...
\begin{example}
  ...
  \newthought{Step 1. Let's first show that $\R P^n$ is a topological $n$-manifold}.
...
\begin{example}
  ...
  We can describe the cone with the following atlas $\cA = \{(U_i,\phi_i)\mid i=1,2,3\}$:
  \begin{itemize}
    \item $U_1 := \{p\in\cC\mid p^3 < 1\}$ with $x = (x^1, x^2) = \phi_1(p) = (p^1,p^2+1)$, thus
    \begin{equation}
      \phi_1^{-1}(x) = \left(x^1, x^2-1, \sqrt{(x^1)^2 + (x^2-1)^2}\right).
    \end{equation}
    \item $U_2 = \{p\in\cC\mid 1/2 < p^3 \leq 1, \; (p^1, p^2) \neq (0, p^3)\}$ with $\phi_2$ defined as follows. Let
    \begin{equation}
      q = \psi(p) = \left(\frac{p^1}{p^3}, \frac{p^2}{p^3}, p^3\right)
      \quad\mbox{and}\quad
      \sigma(q) = \left(\frac{2q^1}{1-q^2}, 1-q^3\right),
    \end{equation}
    then $x = \phi_2(p) = (\sigma\circ\psi)(p)$ and $\phi_2(U_2) = \R\times[0,1/2) \subset\cH^2$.
    \item $U_3 = \{p\in\cC\mid 1/2 < p^3 \leq 1, \; (p^1, p^2) \neq (0, -p^3)$ and $\phi_3$ defined similarly as in the previous point.
  \end{itemize}
  \begin{marginfigure}
  \includegraphics{1_5_9-cyl2}
  \caption{Compare $\phi$ with the stereographic projections from Exercise~\ref{ex:stereo}. Do you notice any similarity?}
  \end{marginfigure}
  The boundary is given by $\partial\cC = \phi_2^{-1}(\R\times\{0\}) \cup \phi_3^{-1}(\R\times\{0\})$.
\end{example}

\begin{exercise}
  Explicitly define $\phi_3$ from the previous example.
  Why is $\phi_1$ not appearing in $\partial \cC$?
\end{exercise}
...
\begin{exercise}
  ...
  Give a smooth structure to $M$ such that every smooth interior chart is a smooth chart for the standard smooth structure on $\mathring M$. \textit{\small Hint: consider the map $\pi\circ\sigma^{-1}:\R^n\to\R^n$ where $\sigma:\bS^n\to\R^n$ is the stereographic projection from Exercise~\ref{ex:stereo} and $\pi:\R^{n+1}\to\R^n$ is a projection that omits one of the first $n$ coordinates.}
\end{exercise}
