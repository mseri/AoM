...
%
\begin{theorem}[Inverse function theorem for $\R^n$]\label{thm:ift}\idxthm{Inverse function theorem (Euclidean)}
...
\begin{theorem}[Inverse function theorem for manifolds]\label{thm:iftm}\idxthm{Inverse function theorem (manifolds)}
...
\begin{theorem}[Rank theorem]\label{thm:rank}\idxthm{Rank theorem}
...
\begin{definition}
	...
	\begin{itemize}
		\item $F$ is a \emph{immersion}\idxdef{Immersion} if $dF_p$ is injective for all $p\in M$ ($\Rightarrow\; m\leq n$);
		\item $F$ is a \emph{submersion}\idxdef{Submersion} if $dF_p$ is surjective for all $p\in M$ ($\Rightarrow\; m\geq n$);
		\item $F$ is a \emph{embedding}\idxdef{Embedding}\footnote{This is a particular case of a more general concept, the topological embedding, which is defined as an injective continuous map that is a homeomorphism onto its image.} if $F$ is an injective immersion that is also a diffeomorphism onto its image $F(M)\subset N$, where the topology on $F(M)$ is the subspace topology as a subset of $N$.
	\end{itemize}
\end{definition}
...
\begin{definition}
	...
	We say that $M$ is an \emph{embedded (or regular) submanifold}\idxdef{Embedded submanifold} of $N$ if the inclusion $M\hookrightarrow N$ is an embedding. If the inclusion is just an immersion, we say that $M$ is an \emph{immersed submanifold}\idxdef{Immersed submanifold}.
\end{definition}
...
\begin{definition}\idxdef{Regular point}\idxdef{Critical point}\idxdef{Regular value}\idxdef{Critical value}
...
\begin{theorem}[Regular levelset theorem]\label{thm:impl_fun}\idxthm{Regular levelset theorem}\idxthm{Implicit function theorem (manifolds)}
...
\begin{proposition}\label{prop:tangent_levelset}\idxthm{Tangent space to a levelset}
