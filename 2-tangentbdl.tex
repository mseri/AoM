...
With the constructions that we have developed so-far, it should not come a surprise that the notion of differential becomes somewhat tautological.

\begin{defn}
    Let $F: M \to N$ be a smooth map between the smooth manifolds $M$ and $N$.
    Let $p\in M$. The \emph{differential $d F_p$ of $F$ at $p$} is the map\footnote{The differential $dF_p(v)$ is also commonly denoted $F_* v_p$, $DF(p)[v]$ or variations thereof. Unfortunately there is no real conventional notation among those.}
    \begin{equation}
