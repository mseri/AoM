...
For example, since $GL_n(\R)$ is an open submanifolds of the vector space $\mathrm{Mat}(n)$, we can identify its tangent space at each point $X\in GL_n(\R)$ with the full space of matrices $\mathrm{Mat}(n)$.
...
\begin{definition}
  ...
  A point $p\in M$ is said to be a \emph{regular point} of $F$ if $F$ has rank $n$ at $p$, while it is called a \emph{critical point} if it is not.
