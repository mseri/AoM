...
\section{Germs and derivations}\idxdef{Germ}\idxdef{Derivation}
...
\begin{definition}
	...
	We say that two functions $f\in C^\infty(U)$ and $g\in C^\infty(V)$ have the same \emph{germ} at $p$ if there exists a neighbourhood $W\subset U\cap V$ of $p$ where $f$ and $g$ agree, that is, $f|_W = g|_W$.
\end{definition}
\begin{marginfigure}
...
\begin{definition}
	...
	A \emph{tangent vector at $p$}\idxdef{Tangent vector} is a linear map
	...
	Since a tangent vector is a linear map from the vector space $C_p^\infty(M)$ to $\R$, the set of all tangent vectors at a point $p$ is itself a vector space\footnote{Exercise: why is this true?} which we denote $T_p M$ and call \emph{tangent space}\idxdef{Tangent space}.
\end{definition}
...
\begin{definition}\idxdef{Derivation at a point}
	Let $M$ be a smooth manifold and $p\in M$.
...
\begin{tcolorbox}
	First of all, from now on we are free to interpret tangent vectors in $T_p M$ as derivations of $C^\infty(W)$ at $p$ for \emph{any} open set $W\subseteq M$ containing $p$.

	In particular, it is often convenient to have $W$ coincide with the domain of a chart or with the whole manifold $M$.
\end{tcolorbox}
...
\begin{example}\label{ex:partialderivative}
	...
	\end{align}
	It is already becoming more and more convenient to draw commutative diagrams to see ``how things are moving around'':
	\begin{equation}
...
\begin{example}\label{ex:vec:chg_coords_polar}
	The transition map between the standard Euclidean coordinates and the polar coordinates on appropriate open sets in $\R^2$ is given by $(x,y) = \tau(\rho,\theta) = (\rho\cos(\theta), \rho\sin(\theta))$. Let $p\in\R^2$ denote the point with coordinates $(\rho, \theta) = (3, \pi)$ and $v\in T_p\R^2$ the tangent vector with polar coordinate representation
	\begin{equation}
...
\begin{definition}\label{def:differentialMap}\idxdef{Differential (of a map)}
...
\begin{theorem}[The chain rule on manifolds]\label{thm:chainrule_mfld}\idxthm{Chain rule (manifolds)}
...
\begin{example}
	...
	\end{equation}
	as in the example.
	If the theory developed here is correct, we should be able to recover the vector in cartesian coordinates as the vector in $T_{\tau(p)}N$ obtained by computing $d\tau_p(v)$.

	Applying $d\tau_p$ to $v$ we get
	\begin{align}
		d\tau_p (v)
		 & = 5 d\tau_p \left(\frac{\partial}{\partial \rho}\Big|_p\right) - d\tau_p \left(\frac{\partial}{\partial \theta}\Big|_p\right)                                                                                                        \\
		 & = 5 \left(\frac{\partial\, \rho \cos(\theta)}{\partial \rho}\Big|_p \frac{\partial}{\partial x}\Big|_{\tau(p)} + \frac{\partial\, \rho \sin(\theta)}{\partial \rho}\Big|_p \frac{\partial}{\partial y}\Big|_{\tau(p)}\right)
		\\
		 & \quad - \left(\frac{\partial\, \rho \cos(\theta)}{\partial \theta}\Big|_p \frac{\partial}{\partial x}\Big|_{\tau(p)} + \frac{\partial\, \rho \sin(\theta)}{\partial \theta}\Big|_p \frac{\partial}{\partial y}\Big|_{\tau(p)}\right) \\
		 & = 5 \left(-1 \frac{\partial}{\partial x}\Big|_{\tau(p)} + 0 \frac{\partial}{\partial y}\Big|_{\tau(p)}\right)
		- \left(0 \frac{\partial}{\partial x}\Big|_{\tau(p)} - 3 \frac{\partial}{\partial y}\Big|_{\tau(p)}\right)                                                                                                                              \\
		 & = -5 \frac{\partial}{\partial x}\Big|_{\tau(p)} + 3 \frac{\partial}{\partial y}\Big|_{\tau(p)},
	\end{align}
	...
	\begin{equation}
		...
		\end{pmatrix}.
	\end{equation}
	Thus, by Proposition~\ref{prop:DiffCoords}, the matrix of $d\tau_p$ with respect to the bases $\left\{\frac{\partial}{\partial \rho}\big|_p, \frac{\partial}{\partial \theta}\big|_p\right\}$ of $T_pM$ and $\left\{\frac{\partial}{\partial x}\big|_{\tau(p)}, \frac{\partial}{\partial y}\big|_{\tau(p)}\right\}$ of $T_{\tau(p)}N$ is given by $D\tau(p)$.
	This means that we should be able to recover $d\tau_p(v)$ by multiplying the matrix $D\tau(p)$ by the coordinate vector of $v$ with respect to the basis $\left\{\frac{\partial}{\partial \rho}\big|_p, \frac{\partial}{\partial \theta}\big|_p\right\}$, which is $(5, -1)$.
	Indeed,
	\begin{equation}
		D\tau(p) v =
		\begin{pmatrix}
...
\begin{definition}\label{def:tg:ascurvespeed}\idxdef{Tangent vector}
...
\section{The tangent bundle}\label{sec:tangentbundle}\idxdef{Tangent bundle}
