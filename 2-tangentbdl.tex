...
\begin{lem}
    ...
    Denote by $\underline c$ the germ of a constant function $(U, p \mapsto c)$.
    Then $v(\underline c) = 0$.
\end{lem}
\begin{proof}
    Since $\underline c = c \underline 1$, by linearity we have $v(\underline c) = c v(\underline 1)$.
    Thus, it will be enough to show that $v(\underline 1) = 0$.
    ...
    \begin{equation}
        v(\underline 1) = v(\underline 1 \underline 1) = 2 \eval_p(\underline 1)v(\underline 1) = 2 v(\underline 1).
    \end{equation}
    Thus, $v(\underline 1) = 0$, concluding the proof.
\end{proof}
...
If $v\in T_p M$, then we already saw that $v$ naturally defines a derivation $w$ of $C^\infty(W)$ for any open neighborhood $W$ of $p$. In this case $w(f) := v(\underline f)$.
