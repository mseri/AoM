...
In this chapter we will see exactly how to do that: we will describe how to associate to each point $x\in M$ on an $n$-dimensional smooth manifold $M$ an $n$-dimensional vector space $T_x M$ called \emph{tangent space to $M$ at $x$}.
For a manifold embedded into a euclidean ambient space this tangent space will coincide with the intuitive understanding of a tangent hyperplane to the point on the manifold, see also Figure~\ref{fig:tan-embedded-sphere}, however it will take a couple of chapters before we can show that it is the case.
