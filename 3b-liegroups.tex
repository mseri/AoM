...
\begin{proof}
  ...
  \begin{equation}\nonumber
    \begin{tikzcd}[row sep=large, column sep=large]
      T_0\fg \arrow[rr, "d\exp_0"] & & \fg \\
      & \fg \arrow[ul, "\tau_0"] \arrow[ur, "\id" below] &
    \end{tikzcd}
  \end{equation}
...
\begin{remark}
  Note that it we have not shown $\exp(X + Y) = (\exp X)(\exp Y)$. In fact, this is \emph{false} in general.
\end{remark}
...
We will come back to discuss Lie groups and Lie algebras in the appendix on distribution theory and Frobenious theorem.