...
\marginnote{Let's denote the two charts respectively by $\varphi$ and $\psi$, then if $\varphi = \psi\circ\varphi^{-1}$ is the corresponding transition map, one has \begin{equation}
  \frac{\partial y^j}{\partial x^i}(p) \frac{\partial}{\partial y^j}\Big|_p = (D\varphi(p))_i^j \frac{\partial}{\partial y^j}\Big|_p,
\end{equation}
where $D\varphi$ is the Jacobian matrix of the transition map.}
\begin{equation}
