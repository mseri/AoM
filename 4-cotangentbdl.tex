...
\begin{definition}\label{def:pullback:oneform}
  ...
  The \emph{pullback} of covectors by $F$ at the point $F(p)$, is the dual linear map of the diffetential
  ...
  \end{equation}
  defined by duality in the following way\footnote{Or, omitting the point of application, $\left(F^*\omega, v\right) := \left(\omega, dF(v)\right)$.}:
  \begin{equation}
    \left(dF^*_p\omega, v\right) := \left(\omega, dF_p(v)\right),\quad
...
\begin{proof}
  ...
  % The atlas is obtained by an atlas $\{(U_i, \phi_i)\}$ of $M$ by defining the new atlas
  % \begin{equation}
  %   \left\{ T^* U_i, \left(\phi_i, d(\phi_i^{-1})^*\right)\right\}
  % \end{equation}
  % where
  % \begin{align}
  %   \left(\phi_i, (\phi_i^{-1})^*\right) : T^* U_i &\to T^*\phi(U_i),\\
  %   (p, \omega) &\mapsto \left(\phi_i(p), d(\phi_i^{-1})^*)_p\omega \right).
  % \end{align}
  \begin{exercise}
...
\begin{exercise}\label{ex:propdiff}
  ...
  \begin{equation}
    F^* df = d (f\circ F) =: d (f^* g),
  \end{equation}
  where we introduced the \emph{pullback} of the function $f$ as $f^*g := g\circ f$.

  Hint: apply the equations at a point $p\in M$ and keep rewriting the equations in different forms.
\end{exercise}
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \end{equation}
    Hint: use the chain rule to get $\delta'(t) = \gamma'(F(t))F'(t)$ and then apply \eqref{eq:lineIntCurve}.
    \item Let $f\in C^\infty(M)$. Prove the fundamental theorem of calculus
    \begin{equation}
      \int_\gamma df = f(\gamma(b)) - f(\gamma(a)).
    \end{equation}
    Hint: justify that $df_{\gamma(t)}(\gamma'(t)) = \frac{d}{ds}f(\gamma(s))\big|_{s=t}$ and then use the usual fundamental theorem of calculus on $\R$.
  \end{enumerate}
\end{exercise}

\begin{exercise}[One-forms in thermodynamics]
  Consider a physical system composed of a fixed number of particle.
  ...
  The $1$-form $TdS$ os called the \emph{heat} absorbed by the system while $-d dV$ os the \emph{work} performed by the system.

  Differently from the other properties of the system, these are not functions and thanks to this it makes sense to ask how much heat has been transferred or how much work has been performed: i.e. the integrals of those one-forms over curves in the space of equilibrium states.
