...
\begin{exercise}\label{ex:propdiff}
 ...
 {\small Hint: apply the equations at a point $p\in M$ and keep rewriting the equations in different forms.}
\end{exercise}
...
\begin{example}[The pullback bundle]
  ...
  If $\Phi : \pi^{-1}(U) \to U\times\R^r$ is a bundle diffeomorphism for $E$, then $\Phi\circ\pi_2: \Pi_1^{-1}(F^{-1}(U)) \to U\times\R^r$ is a bundle diffeomorphism for $F^*E$.
...
\begin{exercise}
  ...
  \begin{enumerate}
    ...
    \marginnote{This shows that line intefrals are independent of the parametrization.}
    \begin{equation}
      \int_\delta \omega = \int_\gamma \omega.
    \end{equation}
   {\small Hint: use the chain rule to get $\delta'(t) = \gamma'(F(t))F'(t)$ and then apply \eqref{eq:lineIntCurve}.}
    \item Let $f\in C^\infty(M)$. Prove the fundamental theorem of calculus:
    \begin{equation}
      \int_\gamma df = f(\gamma(b)) - f(\gamma(a)).
    \end{equation}
   {\small Hint: justify that $df_{\gamma(t)}(\gamma'(t)) = \frac{d}{ds}f(\gamma(s))\big|_{s=t}$ and then use the usual fundamental theorem of calculus on $\R$.}
  \end{enumerate}
\end{exercise}
