...
The difference in the way vector and covector transform is also reflected in the way they are transformed by smooth maps between manifolds.
...
\begin{definition}\label{def:pullback:oneform}
  ...
  \begin{equation}
    \left(F^*\omega_p, v\right) := \left(\omega_{F(p)}, dF_p(v)\right),\quad
...
\noindent Let's check that the definition above makes sense: $F^* \omega_p \in T^*_p M$ so $v\in T_p M$, but $\omega_{F(p)}\in T^*_{F(p)}N$ so $df_p(v)\in T_{F(p)}N$ since $df_p: T_pM\to T_{F(p)}N$.
...
\todo{Cotangent Bundle + forms as sections of the cotangent bundle + Example Liouville forme (Tu 17.4) + proposition 11.25 from Lee}